...
(function () {
  ...
  const SHIP_ART = new Set([
    ...
    'torrent','vertex','zenith','zodiac','aeon',
    // Bioficer May-2026 ships + previously art-less hulls/cells (desktop had these; mobile hid them)
  ...
  async function renderMobileRules() {
    ...
    el.innerHTML = `
      <div class="rules-intro">
        ...
        <a class="btn btn-outline btn-block" href="../assets/rules/game-summary-dfc.pdf" target="_blank" rel="noopener">Game Summary</a>
  ...
  // The six-word phrase this device can hand to another one, if any. A Discord
  // device still carries the Sync Token it had before signing in.
  function syncPhrase() {
  ...
  function syncOnHTML() {
    ...
    const tokenPart = !phrase ? '' : `
      ...
      <code class="m-sync-token" id="m-sync-token">${esc(phrase)}</code>
