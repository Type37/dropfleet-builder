...
def special_rules():
    """Every Ship and Weapon Special Rule (rulebook 14.1, 14.2), name -> verbatim text, for the rule chips."""
    wiki = json.load(io.open(os.path.join(ROOT, 'data', 'rules-wiki.json'), encoding='utf-8'))
    out = {}

    def walk(n):
        if isinstance(n, dict):
            if n.get('number') in ('14.1', '14.2'):
                for c in n.get('children', []):
                    out[c['heading']] = '<br>'.join(runs_html(b.get('runs', [])) for b in c.get('body', []) if b.get('kind') != 'table')
                return
            for v in n.values():
                walk(v)
        elif isinstance(n, list):
    ...
    walk(wiki)
    if not out:
        sys.exit('rules-wiki.json has no 14.1/14.2 special rules')
    return out
