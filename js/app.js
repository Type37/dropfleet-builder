...
let activeGroupId = null;
  ...
  async function init() {
    ...
    if (discordDone) {
      discordDone.then(r => {
        ...
        renderFleetList();
        installSyncHook();
        showToast('Signed in as ' + r.name + '. ' + r.total + ' fleet' + (r.total === 1 ? '' : 's') + ' syncing');
      }).catch(e => showToast(e.message || 'Discord sign-in failed'));
      return;
    }
          ...
          <div class="rules-doc">
            <div class="rules-intro">
              ...
              <span class="rules-intro-acts">
                <a class="btn btn-outline" href="assets/rules/game-summary-dfc.pdf" target="_blank" rel="noopener">Game Summary</a>
    ...
    const mini = {
      ...
      g: fleet.battleGroups.map(g => ({
        ...
        sh: g.ships.map(s => {
          ...
          if (s.feature2) entry.ft2 = s.feature2;
  ...
  // ship card with the full ship card). The admiral character is managed in the
    ...
    <div class="group-ships-list">
      <div class="group-ship-entry">
        ...
        <div class="ship-card-body" style="flex:1;min-width:0;display:flex;flex-direction:column;gap:var(--sp-sm)">
          ${sharedShipCard(currentFleet, a, fdb)}
  ...
  function renderDetailPanel() {
    ...
    // like a normal battlegroup (its full ship card in the detail panel).
  ...
  function renderWeaponRow(w, omitName, withCalc) {
    ...
    // Impel-X, Burnthrough-X). Same logic as the print ship card.
  ...
  // Bombardment Torpedoes, Launch 2, Limited-6).
  function renderFeatureLoads(feat) {
    return (feat.loads || []).map(l =>
      `<div class="feature-weapon">Launch ${esc(String(l.launch))} ${esc(String(l.name))}${l.special && l.special !== '-' ? ` ${renderWeaponSpecialChips(l.special)}` : ''}</div>`
    ).join('');
  }
  function renderFeatureStats(feat) {
    ...
    const weapons = renderFeatureWeapons(feat) + renderFeatureLoads(feat);
  ...
  // A Feature Carrier's chosen features, one entry per slot. Slot 1 lives on
  // `ship.feature` (the original single-slot field, kept so old saves and shared
  // links still load); slot 2 on `ship.feature2`.
  const FEATURE_SLOT_KEYS = ['feature', 'feature2'];
  function featureSlotCount(dbShip) {
  ...
  function renderFeatureCarrierBlock(ship, dbShip, groupId, readOnly) {
    ...
    const row = (slot, value, name, cost, feat, isChosen) => {
      ...
      return `<label class="feature-radio${isChosen ? ' selected' : ''}">
        <input type="radio" name="feat-${ship.id}-${slot}"${isChosen ? ' checked' : ''} onchange="App.changeFeature('${groupId}','${ship.id}','${value.replace(/'/g, "\\'")}',${slot})">
        ${art ? `<img class="feature-radio-art" src="${art}" alt="" loading="lazy" onerror="this.remove()">` : ''}
        <span class="feature-radio-main">
    ...
    // the game), so the picker never marks them required and always offers
    // "No feature". Ships that choose two (the Resistance Battle Carriers) get one
    // independent list per slot, and duplicates across slots are allowed.
    const blocks = [];
    for (let slot = 0; slot < slots; slot++) {
      const chosen = ship[FEATURE_SLOT_KEYS[slot]] || '';
      const label = slots > 1
        ? `Deployable Feature ${slot + 1} of ${slots}, optional`
        : 'Deployable Feature, optional';
      const noneRow = row(slot, '', 'No feature', 0, null, chosen === '');
      const featRows = feats.map(f => row(slot, f.name, f.name, f.cost, f, f.name === chosen)).join('');
      blocks.push(`<div class="feature-carrier-label">${label}</div>
        <div class="feature-radio-list">${noneRow}${featRows}</div>`);
    }
    return `<div class="feature-carrier-block">${blocks.join('')}</div>`;
  }
  ...
  function renderSystemsPicker(ship, dbShip, groupId, factionKey, readOnly) {
    ...
    const body = cats.map(cat => {
      ...
      const rows = opts.map(o => {
        ...
        // weapon ship card, launch options get the launch-asset ship card (Fighters/
        ...
        // The option name already heads the card, so a single-weapon ship card
  ...
  function renderGroupShipEntry(ship, dbShip, groupId, count = 1) {
    ...
    // York) read correctly. Each option's own ship card appears only on the
    ...
    // BOTH options as radio cards, each with its full weapon ship card, and pick
    ...
    // Render an option's weapon + launch ship card. In a multi-option picker we
      ...
      loadoutsHtml = loadoutOpts.map((lo, loIdx) => {
        ...
        if (lo.options.length > 1) {
          const cards = lo.options.map((opt, oi) => {
            const on = oi === selIdx;
            const costLabel = opt.cost > 0 ? `+${opt.cost} pts` : opt.cost < 0 ? `${opt.cost} pts` : 'Included';
            // Don't repeat the option name when its weapon ship card already shows
            // it (e.g. option "Cobra Heavy Laser Pair" over a Cobra Heavy Laser Pair
            // weapon row). The selected option shows no ship card (it's in the main
  ...
  function changeFeature(groupId, shipId, featureName, slot) {
    if (!currentFleet) return;
    const group = currentFleet.battleGroups.find(g => g.id === groupId);
    if (!group) return;
    const ship = group.ships.find(s => s.id === shipId);
  ...
  function recalcShipPoints(ship, dbShip, factionKey) {
    ...
    // Deployable features (e.g. a Porter's Genitor Tower) add their own cost. A
    // Battle Carrier carries two, so every slot is counted.
    const feats = (shipDB[factionKey] && shipDB[factionKey].deployableFeatures) || [];
    FEATURE_SLOT_KEYS.forEach(k => {
      if (!ship[k]) return;
      const f = feats.find(x => x.name === ship[k]);
      if (f) total += f.cost || 0;
    });
    return total;
  }
    ...
    let html = admirals.map((a, i) => {
      ...
      if (a.type === 'Famous' && a.shipKey) {
        ...
        if (flagship) {
          ...
          // opens the full ship card. The admiral card here just links to it.
  ...
  function buildFullPrintHTML(f) {
    ...
    if (!noAbilities) {
      ...
      // Each group's label (Captain, Core Abilities…) runs up the left edge beside
      // its own rows, so the groups read apart without a heading row between them.
      const abilBody = fleetAbilityGroups(f).map(g =>
        g.rows.map((r, i) => `<tr class="${i === 0 ? 'dp-abil-first' : ''}${r.pick === 'on' ? ' dp-abil-on' : ''}">${i === 0 ? `<th class="dp-abil-group" rowspan="${g.rows.length}" scope="rowgroup"><span>${esc(g.label)}</span></th>` : ''}<td class="dp-abil-name">${box(r)}${esc(r.name)}</td><td class="dp-abil-cost">${esc(r.cost || '')}</td><td class="dp-abil-effect">${ruleHtml(r.effect || '')}</td></tr>`).join('')
      ).join('');
  ...
  // ── Shared Fleet Viewer ──
  // One ship's ship card body for the shared-fleet preview: effective stats,
  ...
  function sharedShipCard(fleet, ship, dbShip) {
    if (!dbShip) return '';
    let h = '';
  ...
  function showSharedFleet(fleet) {
    ...
    if (fleet.admirals && fleet.admirals.length > 0) {
      html += `<div class="shared-section">
        ...
        ${fleet.admirals.map(a => {
          ...
          // Famous admirals fly a flagship — show its full ship card like the rest
    ...
    fleet.battleGroups.forEach((g, i) => {
      ...
      Object.values(profiles).forEach(({ ship, count }) => {
        ...
        if (dbShip) html += sharedShipCard(fleet, ship, dbShip);
  ...
  const CHANGELOG = [
    ...
    { date: '2026-09-13', title: 'Scenario Generator: Location', items: [
      'A rolled scenario can say where the battle is fought: one of 37 orbital places from the Commander Universe lore, from Earth Orbit and the Moon to Styx, the Tlalocan Moons and the Colonies, each with a short account of what happened there.',
    ...
    { date: '2026-09-11', title: 'Scenarios: every published scenario, and a separate generator', items: [
      ...
      'A Game Summary card on the home page opens Andrew Yep’s three-page summary of the round sequence, core abilities, Dropsites and ship special rules.',
      'Scenario pages and the generator have a score sheet in place of the round and VP bar: tick or count each scoring rule, with its VP from the rule itself, choose the round, and switch between players for a running total.',
      'Generator Variants follow the book more closely: Gridlocked only arms the Medium Cities it turns into Stations, Secure Comms Array falls back to Small Cities when there is no Medium City, and the Variant text is word for word. Scenario pages read in a clearer order: the objective first, then the Standard Scoring it uses, under a middle heading step; Roboto Slab is kept for titles and headings.',
  ...
  function syncAccountHTML() {
    ...
    if (d) {
      return `<div class="sync-account sync-account-on">
        <div class="sync-account-row">
          ${d.avatar ? `<img class="sync-avatar" src="${esc(d.avatar)}" alt="" referrerpolicy="no-referrer">` : ''}
          <div class="sync-account-who">
            <div class="sync-account-name">${esc(d.name)}</div>
            <div class="sync-account-mail">Discord</div>
          </div>
          <button class="btn btn-outline btn-sm" id="sync-discord-out">Sign out</button>
        </div>
      </div>`;
    }
    ...
    if (!u) {
      return `<div class="sync-account">
        ...
        ${discord ? `<button class="sync-discord-btn" id="sync-discord">${DISCORD_ICON} Sign in with Discord</button>` : ''}
        ${google ? `<div id="sync-google-btn" class="sync-google-btn"></div>` : ''}
      </div>
  ...
  function syncOnHTML() {
    ...
    // In account mode (Google or Discord) there is no "stop syncing here":
    // signing out is that button, in the account block. The phrase shows only
    // when there is one to share: a Discord device still carries the Sync Token
    // it had before signing in, and other devices can keep using it.
    const phrase = syncPhrase();
    const tokenPart = !phrase ? '' : `
      <div class="settings-group-title">Your Sync Token</div>
      <div class="sync-token-row">
        <code class="sync-token" id="sync-token-text">${esc(phrase)}</code>
  ...
  // The six-word phrase this device can hand to another one, if any.
  function syncPhrase() {
    if (FleetSync.discordUser()) return FleetSync.linkedToken();
    if (FleetSync.mode() === 'account') return null;
    return FleetSync.token();
  }

  function syncDiscordSignOut() {
    confirmAction('Sign out?', 'Your fleets stay on this device, and your online copy is kept. Sign in with Discord again any time to sync them.',
      () => { FleetSync.discordSignOut(); renderSyncPanel(); showToast('Signed out'); },
      { label: 'Sign out', danger: false });
  }
