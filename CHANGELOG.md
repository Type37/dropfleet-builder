...
matches any rule whose name starts "Feature Carrier", so the M-Type Barge's
"Feature Carrier (Outpost)" and the EX-7 Packet Runner count too, not only the
ships that choose a feature. Like the other chips it only shows when the faction
has a match.
...
### 2026-09-13: Scenario refs match the Dropzone layout
...
Carrier (470m, 3,441,000m³). Shaltari Helium Voidflyer (504m, 1,628,000m³). PHR Harpocrates Guerrilla Lighter
(429m, 1,854,000m³). Scourge Hiruko Boarding Cutter
...
one, so a browser treats them as one site and hands them one pile of storage.
Both apps were syncing to the same document under the same key: turning sync on
in either turned it on in the other, and the two games merged into a single list,
so armies appeared here as fleets with no ships. Sync carries the list without
ever looking inside it, which is exactly why it could not tell one game from the
other. The Dropzone app now has its own storage keys and its own sync document.
Armies that already reached a fleet list are removed on first open and on the
next sync, so they do not return on other devices. Existing tokens keep working.
...
- Found and left in place (harmless, matches the new mechanism if ever revived): desktop's sidebar battlegroup nav list (`#groups-nav`) is dead code from an earlier layout, because the element it targets no longer exists in the current builder, which now uses only the center overview panel.
